...
public class BinaryHeap<Value: Comparable>
{
    ...
    public init(comparatorBlock: @escaping (Value, Value) -> Bool) {
        self.root = nil
        self.nodeCount = 0
...
func sortKSortedArray(_ array: inout [Int], _ k: Int) -> [Int]
{
    ...
    var i = 0
    var j = 0
    while i < k && i < array.count {
        minHeap.insert(array[i])
        i += 1
    }
    while i < array.count && j < sortedArray.count {
        minHeap.insert(array[i])
        guard let minValue = minHeap.pop() else { return [] }
        sortedArray[j] = minValue
        i += 1
        j += 1
    }
    //flush min heap if applicable
    while minHeap.count() > 0 && j < sortedArray.count {
        guard let minValue = minHeap.pop() else { return [] }
        sortedArray[j] = minValue
        j += 1
    }
...
var arrayOne: [Int] = [6, 5, 3, 2, 8, 10, 9]
print(sortKSortedArray(&arrayOne, 3))
